...
exports.getCharacterDetail = async (req, res) => {
    ...
    let characterMovies = await db.query('SELECT mo.title,mo.movieImage FROM cast AS ca, movies AS mo WHERE ca.character_id = ? AND ca.movie_id = mo.movie_id ', {
        type: QueryTypes.SELECT,
        replacements: [id]
    })
...
exports.searchCharacters = async (req, res) => {
    ...
    if (req.query.movie) {
        let response = await db.query('SELECT mo.title, ch.name FROM movies AS mo, characters AS ch, cast AS ca WHERE ca.movie_id = ? AND ca.movie_id = mo.movie_id AND ch.character_id = ca.character_id', {
            type: QueryTypes.SELECT,
